...
const CompleteQuestion = ({ questionsData }) => {
  ...
  const handleAnswerSubmit = () => {
    ...
    if (Array.isArray(correctVal)) {
      const userWords = trimmedAnswer.toLowerCase().split(",").map((w) => w.trim());
        ...
        <div className="complete-quiz__container">
          ...
          {isAnswered && (
            <div className={`complete-quiz__feedback mb-4 animate-fade-in`}>
              ...
              >
                {Array.isArray(currentQuestion.correctAnswer) ? (
                  userAnswers[currentQuestionIndex]?.toLowerCase().split(",").map((w) => w.trim()).sort().join(",") ===
                  currentQuestion.correctAnswer.map((w) => w.toLowerCase()).sort().join(",")
                ) : (
                  userAnswers[currentQuestionIndex]?.toLowerCase() ===
                  currentQuestion.correctAnswer.toLowerCase()
                ) ? (
