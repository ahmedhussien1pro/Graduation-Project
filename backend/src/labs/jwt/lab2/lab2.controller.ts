...
import { comparePasswords, createJWT } from './lab2.service';
import prisma from '../../../utilities/db';

export const loginController = async (req: Request, res: Response) => {
    const user = await prisma.lab2jwt.findUnique({
        where: {
...
export const createUser = async (req: Request, res: Response) => {
    try {
        ...
        const existingUser = await prisma.lab2jwt.findUnique({
            where: { username },
        });
        ...
        const newUser = await prisma.lab2jwt.create({
